...
const actionType = `${FeatureKeys.Images}/submitDelete`

export const submitDelete = createAsyncThunk<Responses.DeleteImage, string>(actionType, async (id: string, thunkApi) => {
    try {
        ...
        const response = await client.deleteImage(id)
        if (response?.success) {
            return response
        } else {
